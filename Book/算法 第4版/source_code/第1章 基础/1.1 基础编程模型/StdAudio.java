 ...
 * reading and writing audio files and playing audio files in a background
 * thread.
 * <p>
 * <b>Getting started.</b>
 ...
 * On some systems, your Java program may terminate before all of the samples
 * have been
 * played. To prevent this, it is recommend that you call one of the following
 * two
 ...
 * </ul>
 * <p>
 * The first method drain any samples queued to the sound card that have not yet
 * been played.
 * The second method also closes standard audio so that it cannot be used again
 * in the same
 * program.
 * <p>
 * <b>Reference.</b>
    ...
    public static double[] read(String filename) {
        ...
        try {
            ...
            if (frameLength * frameSize >= Integer.MAX_VALUE) {
                throw new IllegalArgumentException(
                        "the audio file is too large to store in memory: " + (frameLength * frameSize));
            }
        ...
        // big endian, stereo
            ...
            double[] data = new double[n / 4];
            for (int i = 0; i < n / 4; i++) {
                double left = ((short) (((bytes[4 * i + 0] & 0xFF) << 8) | (bytes[4 * i + 1] & 0xFF)))
                        / ((double) MAX_16_BIT);
                double right = ((short) (((bytes[4 * i + 2] & 0xFF) << 8) | (bytes[4 * i + 3] & 0xFF)))
                        / ((double) MAX_16_BIT);
                data[i] = (left + right) / 2.0;
            }
            return data;
